...
canvas.grid(columnspan=20, rowspan=10)


# canvas.pack()

fr = open("snail.txt", "r", encoding="utf-8")
...
def draw_cells(old_field, win_size=30):
    ...
    for y in range(height):
        for x in range(width):
            if old_field[y][x] == 1:
                cell_list.append(canvas.create_oval(x*win_size, y*win_size, (x + 1)*win_size, (y+1)*win_size, fill = "green"))
...
#vytvori novy 2rozmerny zoz plny 0
...
def generations_button1():
    ...
    old_field = new_field.copy()
    new_field = create2Dmatrix(width, height)
    canvas.after(100,generations_button1)

button1 = tk.Button(win,command=lambda:generations_button1(),text="AUTOMATIC", width= 20, height= 2, font="Arial",bg="green")
button1.grid(column=8,row=10)


def generations_button2():
    global old_field, new_field
    draw_cells(old_field)
    print(old_field)
    rewrite(old_field, new_field)
...
button2 = tk.Button(win,command=lambda:generations_button2(), text="STEP BY STEP", width= 20, height= 2, font="Arial",bg="green")
button2.grid(column=11,row=10)
